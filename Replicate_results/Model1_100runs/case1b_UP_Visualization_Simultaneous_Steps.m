...
% DIRICHlet 
load('1b_learn_cosines_matrices_up_only.mat')
load('1b_learn_cosines_matrices_down_only.mat')
...
CQ_allUP_mean_table = CQ_allUP_mean_table(index_positive_100,:);
CQ_allDOWN_mean_table = CQ_allDOWN_mean_table(index_positive_100,:);
CQ_allUP_std_table = CQ_allUP_std_table(index_positive_100,:);
CQ_allDOWN_std_table = CQ_allDOWN_std_table(index_positive_100,:);

%EFE
load('EFEpolup_table_reproducibility_100rnf.mat');
load('EFEpoldown_table_reproducibility_100rnf.mat')

EFE_up_mean = mean(EFEpolup_table(index_positive_100,:));
EFE_down_mean = mean(EFEpoldown_table(index_positive_100,:));
EFE_up_std = std(EFEpolup_table(index_positive_100,:));
EFE_down_std = std(EFEpoldown_table(index_positive_100,:));

% VFE

load('F_table_reproducibility_100rnf.mat')
VFE_up_mean = mean(F_table(index_positive_100,:));
VFE_up_std = std(F_table(index_positive_100,:));
...
subplot(4,1,1)
plot(mean(thermometer_table(index_positive_100,1:t)',2), LineWidth=2., Color=[0 0 0]);
%xlabel('Time step');
...
sgtitle('Single-Layer Architectures with Early Up-Regulation', fontsize=15)
